...
{
    return g_bHudAdjustMode == BONE_POS || g_bHudAdjustMode == BONE_ROT || g_bHudAdjustMode == BONE_ADJUST_DELTA_POS || g_bHudAdjustMode == BONE_ADJUST_DELTA_ROT;
}
...
shared_str bone_adjust_config_section(const attachable_hud_item* item)
{
    if (item && item->m_parent_hud_item)
        return item->m_parent_hud_item->object().cNameSect();

    // Pooled HUD items can briefly have no owning gameplay object while they
    // are being attached/detached. The HUD section is a safe legacy fallback.
    return item ? item->m_sect_name : shared_str{};
}
...
{
    return model && find_bone_id_ci(model, "base humanlcollarbone") != BI_NONE && find_bone_id_ci(model, "base humanrcollarbone") != BI_NONE;
}
...
void refresh_bone_adjust_callbacks()
{
    ...
    auto apply_section_to_model = [&](IKinematics* model, attachable_hud_item* item, u16 target_idx, bool replacement_hands) {
        ...
        const shared_str config_section = bone_adjust_config_section(item);
        auto& section_data = bone_adjust_section(config_section);
        for (const auto& [bone_name, adjustment] : section_data.bones)
        {
            ...
            {
                context_it->second.section = config_section;
...
void hud_apply_bone_adjustments(IKinematics* model, const attachable_hud_item* item, IKinematics* authoritative_source)
{
    if (!model || !item)
        return;

    const shared_str section = bone_adjust_config_section(item);
    ...
    std::sort(resolved.begin(), resolved.end(), [](const resolved_adjustment& a, const resolved_adjustment& b) { return a.depth < b.depth; });
...
{
    bones.clear();
    if (!g_player_hud || item_idx > 1)
        return;

    IConsole_Command::vecTips tips;
    if (attachable_hud_item* item = g_player_hud->attached_item(item_idx))
        append_model_bone_tips(item->m_model, tips);

    append_model_bone_tips(item_idx ? g_player_hud->Model2() : g_player_hud->Model(), tips);
    std::sort(tips.begin(), tips.end(), [](const shared_str& left, const shared_str& right) { return _stricmp(left.c_str(), right.c_str()) < 0; });
    bones.assign(tips.begin(), tips.end());
}

bool hud_get_bone_adjustment(u16 item_idx, LPCSTR bone_name, Fvector& position, Fvector& rotation, shared_str* config_section)
{
    attachable_hud_item* item = item_idx <= 1 ? bone_adjust_item_for_index(item_idx) : nullptr;
    if (!item || !bone_name || !bone_name[0])
        return false;
    ...
    if (hud_bone_adjustment* adjustment = find_bone_adjustment(section, bone_name))
    {
...
bool hud_set_bone_adjustment(u16 item_idx, LPCSTR bone_name, const Fvector& position, const Fvector& rotation)
{
    ...
    hud_bone_adjustment& adjustment = edit_bone_adjustment(bone_adjust_config_section(item), bone_name);
    adjustment.position = position;
    adjustment.rotation = rotation;
    return true;
}
...
bool set_bone_adjust_mode(LPCSTR bone_name)
{
    ...
    const shared_str config_section = bone_adjust_config_section(item);
    edit_bone_adjustment(config_section, g_bone_adjust_bone.c_str());
    g_bHudAdjustMode = BONE_POS;

    Msg("bone adjust mode: [%s] for weapon section [%s]", g_bone_adjust_bone.c_str(), config_section.c_str());
...
public:
    ...
    void Info(TInfo& info) override { strcpy_s(info, "adjust any HUD/item skeleton bone: bone_adjust_mode <bone_name|off>"); }
...
void player_hud::tune(const Ivector& _values)
{
    ...
    if (is_bone_adjust_mode())
    {
        ...
        const shared_str config_section = bone_adjust_config_section(item);
        hud_bone_adjustment& adjustment = edit_bone_adjustment(config_section, g_bone_adjust_bone.c_str());
        Fvector diff{};
        ...
        if (values.x || values.y || values.z)
            print_bone_adjustment(config_section, g_bone_adjust_bone.c_str(), adjustment);
        return;
    }
...
void hud_draw_adjust_mode()
{
    ...
    if (is_bone_adjust_mode())
    {
        ...
        {
            const shared_str config_section = bone_adjust_config_section(item);
            if (hud_bone_adjustment* adjustment = find_bone_adjustment(config_section, g_bone_adjust_bone.c_str()))
            {
