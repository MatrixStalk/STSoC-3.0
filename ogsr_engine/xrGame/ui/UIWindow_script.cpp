...
void CUIWindow::script_register(lua_State* L)
{
    module(L)[(def("GetARGB", &GetARGB),
              ...
              class_<CUIWindow>("CUIWindow")
                  ...
                  .def("SetAnimationRotation", &CUIWindow::SetAnimationRotation)
                  .def("SetRotation", &CUIWindow::SetRotation)
                  .def("GetRotation", &CUIWindow::GetRotation)
                  .def("SetRotationSpeed", &CUIWindow::SetRotationSpeed)
                  .def("SetRotationSmoothing", &CUIWindow::SetRotationSmoothing)
...
#pragma todo( \
    "KRodin: ивент PDA_CONTACTS_WND_CONTACT_SELECTED нигде не вызывается. Надо доделать по необходимости. Хотя я не очень представляю, для чего он может понадобиться в скриптах.")
                      ...
                      value("INVENTORY_DETACH_CUSTOM_ADDON", int(INVENTORY_DETACH_CUSTOM_ADDON)))])];
}
